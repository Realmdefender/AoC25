...
def max_joltage(banks):
    ...
    for bank in banks:
        ...
        location_of_first = bank.index(first_digit)
        second_digit = max(bank[(location_of_first + 1):])
        joltage_sum += int(first_digit) * 10 + int(second_digit)
...
max_joltage_value = max_joltage(banks)
print(f"The maximum sum of all possible joltage values is: {max_joltage_value}")
...
def max_joltage_extended(banks):
    joltage_sum = 0

    for bank in banks:
        digits = ""
        for i in range(11):
            max_digit = max(bank[:-(12 - i - 1)])
            digit_location = bank.index(max_digit)
            digits += max_digit
            bank = bank[(digit_location + 1):]
...
max_joltage_value_extended = max_joltage_extended(banks)
print(f"The maximum sum of all possible joltage values with 12 batteries is: {max_joltage_value_extended}")